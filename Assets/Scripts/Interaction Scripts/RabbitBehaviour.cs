...
public class RabbitBehaviour : MonoBehaviour
{
    ...
    void Update()
    {
		...
		if(Vector2.Distance(player.transform.position, detectCenter) < 2){
			...
			if(Vector2.Distance(player.transform.position, detectCenter) < 1 && !isBattle){
				...
				isBattle = true;
				PlayerData.currentBattle = new PlayerData.Enemy{name = "Radioactive Rabbit", typeDice = 4, numDice = 1};
